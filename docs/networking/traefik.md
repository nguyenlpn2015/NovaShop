...
cert-manager is active through the reviewed TLS GitOps phase and issues and
renews the environment certificates through ACME HTTP-01. It owns only
certificate lifecycle; Traefik remains the Ingress controller and Argo CD
remains the desired-state reconciler. See the
